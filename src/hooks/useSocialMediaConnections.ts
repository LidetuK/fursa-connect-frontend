  ...
  const disconnectAccount = async (platform: string) => {
    ...
    try {
      ...
      if (platform === 'telegram') {
        // Use Telegram-specific disconnect endpoint
        response = await fetch('https://fursaconnet-production.up.railway.app/telegram/disconnect', {
          method: 'DELETE',
          credentials: 'include',
        });
      } else {
        // Use general social accounts disconnect endpoint
        response = await fetch(`https://fursaconnet-production.up.railway.app/user/social-accounts/${platform}`, {
          method: 'DELETE',
          credentials: 'include',
        });
      }
