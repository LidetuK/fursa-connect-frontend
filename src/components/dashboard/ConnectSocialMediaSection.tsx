...
const ConnectSocialMediaSection = () => {
  ...
  useEffect(() => {
    ...
    if (facebookStatus === 'connected') {
      console.log('Facebook connected successfully, refreshing social accounts');
      loadUserSocialAccounts();
      // Clean up the URL
      window.history.replaceState({}, document.title, window.location.pathname);
    ...
    if (instagramStatus === 'connected') {
      console.log('Instagram connected successfully, refreshing social accounts');
      loadUserSocialAccounts();
      // Clean up the URL
      window.history.replaceState({}, document.title, window.location.pathname);
    } else if (instagramStatus === 'error') {
      console.log('Instagram connection failed');
      // Clean up the URL
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  }, []);
  ...
  useEffect(() => {
    const handleGoogleOAuthComplete = (event: CustomEvent) => {
      console.log('Google OAuth completed, refreshing social accounts:', event.detail);
      loadUserSocialAccounts();
    };

    window.addEventListener('googleOAuthComplete', handleGoogleOAuthComplete as EventListener);

    // Also check URL parameters for Google OAuth completion as fallback
    const urlParams = new URLSearchParams(window.location.search);
    const googleStatus = urlParams.get('google');
    if (googleStatus === 'connected') {
      console.log('Google OAuth detected in URL, refreshing social accounts');
      loadUserSocialAccounts();
    }
  ...
  const handleConnect = async (platformId: string) => {
    ...
    if (platformId === 'facebook') {
      window.location.href = 'https://fursaconnet-production.up.railway.app/auth/facebook';
      return;
    }
    if (platformId === 'instagram') {
      window.location.href = 'https://fursaconnet-production.up.railway.app/auth/instagram';
      return;
    }
    if (platformId === 'telegram') {
      // Handle Telegram connection with prompt
      ...
      try {
        const res = await fetch('https://fursaconnet-production.up.railway.app/telegram/connect', {
          ...
          body: JSON.stringify({ userId: 8, chatId: channelUsername.trim() }), // Use actual user ID
        });
  ...
  const handleDisconnect = async (platformId: string) => {
    if (platformId === 'twitter' || platformId === 'youtube' || platformId === 'linkedin' || platformId === 'facebook' || platformId === 'instagram') {
      try {
    ...
    if (platformId === 'telegram') {
      try {
        const response = await fetch('https://fursaconnet-production.up.railway.app/telegram/disconnect', {
          method: 'DELETE',
          credentials: 'include',
        });
        if (response.ok) {
          // Reload from backend to update the UI
          await loadUserSocialAccounts();
        } else {
  ...
  const handleRefresh = async () => {
    try {
      await loadUserSocialAccounts();
      // Also refresh platform-specific data
      await fetchTwitterUsername();
      await fetchYoutubeChannel();
      await fetchLinkedinName();
    } catch (error) {
