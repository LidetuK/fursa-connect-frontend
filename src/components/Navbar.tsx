...
const Navbar = () => {
  ...
  return (
    <>
      <header className={cn(
        "bg-slate-900/95 backdrop-blur-xl border-b border-orange-500/20 sticky top-0 z-50 transition-all duration-500",
        isScrolled ? "shadow-lg shadow-orange-500/10 py-2" : "py-4"
      )}>
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center">
            ...
            <nav className="hidden lg:flex items-center space-x-1">
              {dynamicFeatures.map((feature, index) => (
                <button
                  ...
                  className="group relative px-3 py-2 rounded-xl hover:bg-orange-500/10 transition-all duration-300 cursor-pointer"
                >
                  <div className="flex items-center gap-2 text-gray-300 group-hover:text-orange-400 transition-colors">
                    <div className="p-1.5 rounded-lg bg-orange-500/20 group-hover:bg-orange-500/30 transition-colors">
                      {feature.icon}
                    </div>
                    <div className="text-sm">
                      <div className="font-semibold">{feature.label}</div>
                      <div className="text-xs text-gray-400 group-hover:text-orange-300">{feature.description}</div>
                    </div>
                  </div>
            ...
            <div className="hidden md:flex items-center space-x-3">
              <Button
                ...
                className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold px-6 py-2.5 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
              >
                Sign In
              </Button>
            </div>
          ...
          {/* Mobile Menu */}
          {isMenuOpen && (
            <div className="lg:hidden pt-6 pb-4 bg-slate-800/95 backdrop-blur-xl rounded-b-2xl mt-4 shadow-xl border border-orange-500/20">
              <div className="grid grid-cols-2 gap-3 px-3 mb-6">
                {dynamicFeatures.map((feature, index) => (
                  <button
                    ...
                    className="flex items-center gap-2 p-3 rounded-xl bg-orange-500/10 hover:bg-orange-500/20 transition-colors cursor-pointer"
                  >
              ...
              <div className="border-t border-orange-500/20 pt-4 mt-4 px-3">
                <Button
                  onClick={openAuthModal}
