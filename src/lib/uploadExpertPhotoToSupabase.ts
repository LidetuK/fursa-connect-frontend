...
export async function uploadExpertPhotoToSupabase(file: File): Promise<string> {
  ...
  // Validate file size (max 2MB - reduced from 10MB for faster uploads)
  const maxSize = 2 * 1024 * 1024; // 2MB
  if (file.size > maxSize) {
    throw new Error('File size must be less than 2MB. Please compress your image or choose a smaller file.');
  }
