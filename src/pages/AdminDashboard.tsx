...
const AdminDashboard = () => {
  ...
  const loadAdminData = async () => {
    try {
      ...
      // Load SMEs from the new sme_registry endpoint
      const smesRes = await fetch('https://fursaconnet-production.up.railway.app/sme_registry');
  ...
  const handleDeleteSme = async (id: number) => {
    ...
    const res = await fetch(`https://fursaconnet-production.up.railway.app/sme_registry/${id}`, { method: 'DELETE' });
    if (res.ok) {
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-medium">
                        {smes.filter(sme => sme.company_logo_url).length}
                      </span>
                      <span className="text-xs text-gray-500">
                        ({smes.length > 0 ? Math.round((smes.filter(sme => sme.company_logo_url).length / smes.length) * 100) : 0}%)
                      </span>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-sm">SMEs with Contact Info</span>
                    <div className="flex items-center gap-2">
                      <span className="font-medium">
                        {smes.filter(sme => sme.phone_number).length}
                      </span>
                      <span className="text-xs text-gray-500">
                        ({smes.length > 0 ? Math.round((smes.filter(sme => sme.phone_number).length / smes.length) * 100) : 0}%)
                      </span>
                    </div>
                  </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                                <div>
                                  ...
                                  <p><strong>Contact:</strong> {expert.phone_number}</p>
                  ...
                  <div className="space-y-6">
                    {smes.map((sme) => (
                      <div key={sme.id} className="bg-white border rounded-lg p-6 shadow-sm flex items-center gap-6">
                        {/* SME Logo */}
                        <div className="flex-shrink-0">
                          {sme.company_logo_url ? (
                            <img
                              src={sme.company_logo_url}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-600"><strong>Name:</strong> {sme.name}</p>
                          <p className="text-sm text-gray-600"><strong>Contact:</strong> {sme.phone_number}</p>
                        </div>
                        <button
                      ...
                      <div key={sme.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500">{sme.phone_number}</div>
                          </div>
                        </div>
                      </div>
                    ))}
